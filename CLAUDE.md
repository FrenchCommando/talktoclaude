...
- Quit from the tray menu. `run.bat` launches the tray app; follow the
  log live with `Get-Content logs\<file> -Wait`. There was a `--console`
  flag until 2026-09-12; removed because the log already had everything
  and `run.bat` never forwarded it anyway. A running app still blocks
  the build, and it has no window to remind you: `tasklist`.
...
- `src/tray.{h,cpp}` — notification-area icon: discs drawn at runtime (no
  ...
  is no console. `--help` alone `AttachConsole`s the parent terminal to
  print usage, and stays silent (never a message box) when there is none,
  so CI's exit-code smoke test can't hang on it. Single instance via a
  named mutex.
...
- `src/logging.{h,cpp}` — timestamped file log, nothing else; `info` and
  `error` are the same sink under two names.
...
- whisper's non-speech markers (`[BLANK_AUDIO]`, `[ Silence ]`) get typed and
  ...
  logs `[nothing said - peak ...]` instead, so a typed
